<?php
	...
	function dispsubcategories($parent_id) {
		...
		while ($row = mysqli_fetch_assoc($select)) {
			echo "<tr><td class='category_title'><a href='/PhpProject8/topics.php?cid=".$row['cat_id']."&scid=".$row['subcat_id']."'>
 ...
 function disptopics($cid,$scid) {
	  include('dbconn.php');
	 $select = mysqli_query($con,"SELECT topic_id, author , title, date_posted , views , replies FROM categories, subcategories, topics WHERE ($cid = topics.category_id) AND ($scid = topics.subcategory_id) AND ($cid = categories.cat_id) AND ($scid = subcategories.subcat_id) ORDER BY topic_id DESC");
	 if(mysqli_num_rows($select) != 0) {
	 echo "<table calss='topics-table'>";
	 echo "<tr><th>Title</th><th>Posted By</th><th>Date Posted</th><th>Views</th><th>Replies</th></tr>";
		while($row = mysqli_fetch_assoc($select))
		{	
			echo "<tr><td><a href='/PhpProject8/readtopic.php?cid=".$cid."&scid=".$scid."&tid=".$row['topic_id']."'>
	  ...
	  echo "<p>this category has no topics yet!<a href='/PhpProject8/newtopic.php?cid=".$cid."&scid=".$scid."'>
	   add the first topic!</a></p>";
 ...
 function replylink($cid, $scid, $tid) {
		echo "<p><a href='/PhpProject8/replyto.php?cid=".$cid."&scid=".$scid."&tid=".$tid."'>Reply to this post</a></p>";
	}
 function replytopost($cid, $scid, $tid) {
		echo "<div class='content'><form action='/PhpProject8/addreply.php?cid=".$cid."&scid=".$scid."&tid=".$tid."' method='POST'>
			  <p>Comment: </p>
